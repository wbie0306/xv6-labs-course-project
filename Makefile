...
# GCC 13 diagnoses xv6's command-dispatch recursion in user/sh.c as infinite.
# It is intentional control flow in the teaching kernel, so retain -Werror for
# all other diagnostics while allowing this compiler-version-specific warning.
...
grade:
	...
	# Invoke the Python harness explicitly so this also works from a Windows
	# checkout mounted in WSL, where the script's shebang may have CRLF endings.
